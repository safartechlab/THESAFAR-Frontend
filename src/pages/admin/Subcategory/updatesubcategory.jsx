...
const Updatesubcategory = () => {
  ...
  if (subcategorydata) {
    setsubcate({
      ...
      category: subcategorydata.categoryID || subcategorydata.category?._id || "", 
      sizes: subcategorydata.sizes?.map((s) => s._id || s) || [],
    });
  }
}, [subcategorydata]);
...
>
  ...
  {category.map((cate) => (
    <option value={cate._id} key={cate._id}>
      {cate.categoryname || cate.category}
    </option>
  ))}
</Form.Select>
            ...
            <div>
              {size.map((s) => (
                <Form.Check
                  ...
                  label={s.size}
                  value={s._id}
                  checked={subcate.sizes.includes(s._id)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setsubcate({
                        ...subcate,
                        sizes: [...subcate.sizes, s._id],
                      });
                    } else {
                      setsubcate({
                        ...subcate,
                        sizes: subcate.sizes.filter((id) => id !== s._id),
                      });
                    }
                  }}
                />
